...
function normalizeNotifications(
  ...
  const scopedNotifications = notifications.filter((notification) => {
    if (options?.includeAdmin) {
      return sameEmail(notification.recipientEmail, recipientEmail) || isAdminNotification(notification);
    }
    return sameEmail(notification.recipientEmail, recipientEmail);
  });

  const notificationByKey = new Map<string, Notification>();
  scopedNotifications.forEach((notification) => {
    const key = getNotificationDedupeKey(notification);
    const current = notificationByKey.get(key);
    if (!current || getNotificationPriority(notification) > getNotificationPriority(current)) {
      notificationByKey.set(key, notification);
    }
  });

  return [...notificationByKey.values()].sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt));
}

function getNotificationDedupeKey(notification: Notification) {
  const relatedKey = notification.relatedEntity && notification.relatedEntityId
    ? `${notification.relatedEntity}:${notification.relatedEntityId}`
    : `notification:${notification.id}`;
  const typeKey = notification.type === "order_created" && isAdminNotification(notification)
    ? "new_order_admin"
    : notification.type;
  return `${notification.recipientEmail}:${typeKey}:${relatedKey}`;
}

function getNotificationPriority(notification: Notification) {
  let priority = Date.parse(notification.createdAt) || 0;
  if (notification.type === "new_order_admin") priority += 1_000_000_000_000;
  if (notification.type === "order_status") priority += 800_000_000_000;
  if (!/\byou\b|\byour\b/i.test(notification.message)) priority += 10_000;
  if (notification.title.toLowerCase().includes("new order")) priority += 1_000;
  return priority;
}
